...
<div class="row center left-align">
    ...
    &nbsp;
    <a href="#download" class="tooltipped" data-tooltip="Download"><i class="material-icons medium blue-text">cloud_download</i></a>
    &nbsp;
...
<div id="modalmovefiles" class="modal">
    ...
    <ul class="collection" id='copyFilesDisplay'>
        ...
        <div class="progress">
            <div class="determinate" style="width: 0%" id="oneFileCopyProgress"></div>
        </div>
    </ul>
    ...
    $(document).ready(function() {
        ...
        $('.sharelink-files').on("click", (function(e) {
            ...
            if ($('input[name="selectedFile"]:checked').length == 0) {
                alert("Nothing to do. No files selected!!!");
            } else {
                var elem = document.getElementById('modalbgworking');
                var instance = M.Modal.getInstance(elem);
                instance.open();
...
/* multiple files move / copy mechanics */
        ...
        $('#copyMultiFileSubmit').on("click", (function(e) {
            e.preventDefault();
            var fileCounter = $('input[name="selectedFile"]:checked').length;
            var count = 0;
            $('input[name="selectedFile"]:checked').each(function() {
                var fileName = this.value;
                $.ajax({
                    url: $('#multiFileForm').attr("action"),
                    type: "POST",
                    data: {
                        '_token': $('input[name=_token]').val(),
                        'current_folder': $('input[name=current_folder]').val(),
                        'fileToCopy': fileName,
                        'targetfoldermulti': $('select[name=targetfoldermulti]').val()
                    },
                    success: function(data) {
                        if (typeof data.progress !== "undefined") {
                ...
                var fileName = this.value;
                $('input[name=fileToCopy]').val(fileName);
                count++;
                var lastFile = 'no';                
                if (count == fileCounter) {
                    lastFile = 'yes';
                }
                $('input[name=lastFile]').val(lastFile);
                var listItem = document.getElementById('oneFileCopy');
                listItem.innerHTML = fileName;
                var progressBar = document.getElementById('oneFileCopyProgress');
