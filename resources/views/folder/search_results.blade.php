...
<div class="row tooltipped"
    data-tooltip="{{count(Storage::disk('local')->allDirectories($directory['folderpath']))}} Dirs/ {{count(Storage::disk('local')->allFiles($directory['folderpath']))}} files">
    ...
    <div class="col s4 right-align">
        <a href="{{route('folder.root', ['current_folder' => '/'.$directory['personalfolderpath']])}}"
            class="tooltipped" data-tooltip="Go to folder location"><i
                class="material-icons blue-text">arrow_forward</i></a>
    </div>
</div>
...
<div class="row">
    ...
    <div class="col s4 right-align">
        ...
        @else
        <a href="{{route('folder.root', ['current_folder' => '/'.$file['filefolder']])}}" class="tooltipped"
            data-tooltip="Go to file location"><i class="material-icons blue-text">arrow_forward</i></a>
        @endif
    </div>
</div>
<div class="row" style="border-bottom: 1px solid gray;">
        ...
        <label>
            <span>
                ...
                <span class="hide-on-med-and-up tooltipped grey-text text-darken-3"
                    data-tooltip="{{$file['filename']}}">{{$file['shortfilename'] . $file['extension']}}</span>
            </span>
        </label>
    </div>
</div>
